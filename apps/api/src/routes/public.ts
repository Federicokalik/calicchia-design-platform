...
import { sql } from '../db';
import { sanitizeBlogHtml, renderBlogContent } from '../lib/html-sanitize';
...
publicRoutes.get('/blog/posts/:slug', async (c) => {
  ...
  return c.json({
    ...
    post: {
      ...
      content: renderBlogContent(post.content as string | null),
...
publicRoutes.get('/blog/rss', async (c) => {
  ...
  return c.json({
    posts: posts.map((p) => ({
      ...
      content: renderBlogContent(p.content as string | null),
      cover_image: resolveImageUrl(p.cover_image as string | null),
      published_at: p.published_at,
      created_at: p.created_at,
      tags: p.tags || [],
