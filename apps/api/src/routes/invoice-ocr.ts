...
const EXTRACT_EXPENSE_PROMPT = `Sei un assistente specializzato nell'estrazione dati da scontrini, ricevute e fatture di spesa italiane.
Analizza l'immagine e restituisci SOLO un JSON valido con questa struttura:

{
  "vendor": "nome del fornitore/negozio",
  "occurred_on": "YYYY-MM-DD",
  "amount": 0.00,
  "vat_amount": 0.00,
  "vat_rate": 22,
  "category": "una di: software | hardware | office | travel | meals | training | marketing | professional_services | utilities | other",
  "description": "descrizione sintetica della spesa",
  "raw_text": "testo grezzo principale per riferimento"
}

Regole:
- Importi come numeri decimali
- Date in formato ISO YYYY-MM-DD
- Categoria: scegli la più appropriata dalla lista, default 'other' se incerto
- Se la VAT non è esplicita, calcolala come amount - amount/(1+vat_rate/100)
...
// POST /api/ai/extract-expense - Extract data from expense receipt/PDF/image
invoiceOcr.post('/extract-expense', async (c) => {
  const contentType = c.req.header('content-type') || '';
  ...
  try {
    ...
    const messages = [
      {
        role: 'user',
        content: [
          { type: 'text', text: EXTRACT_EXPENSE_PROMPT },
          {
            type: 'image_url',
            image_url: {
              url: `data:${mimeType};base64,${base64}`,
    ...
    try {
      ...
      return c.json({ error: 'Impossibile estrarre dati dalla spesa', raw: rawContent }, 422);
    }
