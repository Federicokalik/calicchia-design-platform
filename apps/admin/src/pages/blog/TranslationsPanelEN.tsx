...
const TRANSLATABLE_FIELDS = [
  ...
  { key: 'excerpt', label: 'Excerpt (riassunto)', type: 'textarea', maxChars: 500 },
  { key: 'content', label: 'Contenuto articolo', type: 'textarea', maxChars: 80000 },
] as const;
...
interface TranslationsPanelENProps {
  ...
  /** Riferimento ai valori IT correnti (mostrati come reference). */
  itValues: Translations;
}

/**
 * Pannello traduzioni EN per blog post — pattern identico a portfolio
 * TranslationsPanelEN. 3 campi translatable (title, excerpt, content).
 */
export function TranslationsPanelEN({ postId, itValues }: TranslationsPanelENProps) {
  const queryClient = useQueryClient();
  const [enValues, setEnValues] = useState<Translations>({});

  const { data, isLoading } = useQuery<TranslationsResponse>({
    queryKey: ['blog-post-translations', postId],
    queryFn: () => apiFetch(`/api/blog/posts/${postId}/translations`),
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: Translations) => {
      ...
      return apiFetch(`/api/blog/posts/${postId}/translations/en`, {
        method: 'PATCH',
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['blog-post-translations', postId] });
    },
    onError: (err: Error) => {
  ...
  if (postId === 'new') {
    return (
      <div className="rounded-md border border-dashed border-muted-foreground/30 p-6 text-sm text-muted-foreground">
        ...
        Salva prima l'articolo in italiano per abilitare le traduzioni EN.
      </div>
    );
  }
          ...
          return (
            <div key={f.key} className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <div>
                ...
                {f.type === 'input' ? (
                  <Input value={itVal} readOnly disabled className="mt-1 bg-muted/30" />
                ) : (
                  <Textarea
                    value={itVal}
                    readOnly
                    disabled
                    rows={f.key === 'content' ? 12 : 3}
